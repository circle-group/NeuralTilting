"""
GPU-enabled training orchestration script for tilted stable SDE models.

This is the GPU-aware counterpart to train_tilted_stable_sde.py.  The original
...
----------------------------------------
  ...
  python training/train_tilted_stable_sde_gpu.py \
      --config training_configs/tilted_stable/alpha_1.20_obsstd_0.10_seed_16241891.yaml

  # Continue from parent run
  python training/train_tilted_stable_sde_gpu.py \
      --config training_configs/tilted_stable/alpha_1.20_obsstd_0.10_seed_16241891.yaml \
      --parent-run "alpha_1.20_obsstd_0.10/data_27482070_train_12345678"
"""
...
def _detect_gpu_backend() -> dict:
    ...
    # ------------------------------------------------------------------
        ...
        # A CUDA-enabled jaxlib exposes `jaxlib.cuda_versions` or similar.
        # The most reliable pre-import check is whether the user installed
        # jax via the cuda wheel.
        ...
        if jaxlib_spec is not None:
            try:
                ...
                # This only tells us jaxlib is present; actual GPU presence is
                # verified after JAX is imported below.
            except Exception:
                pass
...
# =============================================================================
# ARGUMENT PARSING
# (identical to train_tilted_stable_sde.py)
# =============================================================================

def parse_arguments():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(
        description="Train tilted stable SDE model with GPU support",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            'Parent run path to continue training from '
            '(e.g., "alpha_1.20_obsstd_0.10/data_27482070_train_12345678")'
        )
    )
...
def load_config(config_path):
    ...
    if config['model_type'] != 'tilted_stable_sde':
...
def validate_parent_run_compatibility(parent_config, current_config):
    ...
    - Model architecture: state_dim, tilting_width/depth, n_time_features
    - Attention architecture: n_attention_references, attention_embed_dim
    - Model parameters: sigma, tau
    ...
    - Learning rate multipliers: lr_multiplier_mlp, lr_multiplier_attention, lr_multiplier_drift
    - Memory management: gc_interval, clear_cache_interval
    """
    errors = []
...
def create_run_directory(alpha, obs_std, data_seed, training_seed, base_dir="training_runs"):
    ...
    Structure:
        training_runs/tilted_stable_sde/alpha_{alpha:.2f}_obsstd_{obs_std:.2f}/data_{data_seed}_train_{training_seed}/

    Returns:
        run_dir (Path): The created run directory
    """
    base_path   = Path(base_dir) / "tilted_stable_sde"
...
def get_parent_run_directory(parent_run_path, base_dir="training_runs"):
    ...
    Args:
        parent_run_path: e.g., "alpha_1.20_obsstd_0.10/data_27482070_train_12345678"
        base_dir: Base directory for training runs

    Returns:
        parent_run_dir (Path): Absolute path to parent run directory
    """
    parent_run_dir = Path(base_dir) / "tilted_stable_sde" / parent_run_path
...
# (identical to train_tilted_stable_sde.py, plus device_backend field)
# =============================================================================
...
def create_metadata(config, training_seed, device_info, parent_run_path=None):
    ...
    Adds a 'device_backend' field (e.g. 'metal', 'cuda', 'cpu') to the
    metadata so that runs can be filtered by compute environment.
    """
...
def prepare_model_params(config, data_dict):
    """Prepare model parameters dictionary for TiltedStableDrivenSDE."""
...
def find_latest_checkpoint(checkpoints_dir):
    ...
    latest_step       = get_step_number(latest_checkpoint)
...
def initialize_or_load_model(config, model_params, parent_run_dir=None):
    ...
    If parent_run_dir is specified:
    1. Try to load from model.eqx (completed training)
    2. If model.eqx doesn't exist, automatically recover from latest checkpoint

    Returns:
        model, model_source, checkpoint_step
    """
    ...
    if parent_run_dir is not None:
        ...
        if parent_model_path.exists():
            ...
            model_source = str(parent_run_dir.relative_to(Path("training_runs") / "tilted_stable_sde"))
        else:
            ...
            model_source = (
                f"{parent_run_dir.relative_to(Path('training_runs') / 'tilted_stable_sde')} "
                f"(checkpoint step {checkpoint_step})"
            )

        template_model = TiltedStableDrivenSDE(**model_params)
        ...
        model        = TiltedStableDrivenSDE(**model_params)
        model_source = 'new'
...
def main():
    ...
    print("=" * 80, flush=True)
    print("TILTED STABLE SDE TRAINING  (GPU-enabled script)", flush=True)
    print("=" * 80, flush=True)
    print(flush=True)
