"""Dense reliability diagram from forecast_samples.pkl files.

Computes coverage at log-spaced nominal levels between 0.5 and 0.999,
loading forecast_samples.pkl directly rather than relying on the fixed
levels stored in metrics.json.

If results look good, the dense levels can later be baked into run_window.py
so this script is no longer needed.

Usage:
    python financial/evaluation/reliability_dense.py \\
...
def load_windows(config_name: str, samples_suffix: str = '') -> list:
    """Load (forecast_samples, forecast_obs, path_weight_threshold) per window."""
    ...
    samples_filename = f'forecast_samples{samples_suffix}.pkl' if samples_suffix else 'forecast_samples.pkl'

    records = []
    for window_dir in sorted(run_dir.glob('window_*')):
        metrics_path = window_dir / metrics_filename
        samples_path = window_dir / samples_filename
        if not metrics_path.exists() or not samples_path.exists():
            continue
        with open(metrics_path) as f:
            m = json.load(f)
        with open(samples_path, 'rb') as f:
            samples = np.array(pickle.load(f))   # (K, H, D)
        obs = np.array(m['forecast_obs'])         # (H, D)
...
def compute_dense_coverage(records: list) -> dict:
    """Compute empirical coverage at DENSE_LEVELS, importance-weighted."""
...
def plot_reliability_dense(ts_cov: dict, gauss_cov: dict, output_path: Path):
    levels  = DENSE_LEVELS
    ts_emp  = [ts_cov[q]    for q in levels]
    g_emp   = [gauss_cov[q] for q in levels]

    fig, ax = plt.subplots(figsize=(6, 5))
    ax.plot([0, 1], [0, 1], 'k--', linewidth=1, label='Perfect calibration')
    ax.plot(levels, ts_emp, 'o-', color='steelblue', linewidth=2,
            markersize=4, label='Tilted Stable')
    ax.plot(levels, g_emp,  's-', color='tomato',    linewidth=2,
            markersize=4, label='Gaussian')

    ax.set_xlabel('Nominal coverage level')
    ax.set_ylabel('Empirical coverage')
    ax.set_title('Reliability diagram (dense)')
    ax.legend()
...
def main():
    ...
    parser.add_argument('--samples-suffix', default='',
                        help='Suffix for forecast_samples and metrics filenames applied to '
                             'both models, e.g. _postfc to load forecast_samples_postfc.pkl.')
    args = parser.parse_args()

    ts_result_name = args.ts_result_name if args.ts_result_name else args.ts_name

    print("Loading windows...")
    ts_records    = load_windows(ts_result_name,  samples_suffix=args.samples_suffix)
