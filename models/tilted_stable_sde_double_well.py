...
class TiltedStableDrivenSDEDoubleWell(eqx.Module):
    """Tilted Stable SDE with a double well (non-linear) drift term.

    Identical to TiltedStableDrivenSDE except the drift is a per-dimension
    ...
    Key components:
    - Drift: Double well potential (bistable, non-linear)
    - Diffusion: Constant diagonal scale factor
    - Tilting: QuadraticNeuralPotential with temporal attention
    """
